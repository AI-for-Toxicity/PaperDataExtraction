...
def parse_extraction_lines(text):
    """
    Extract lines of the form:
    "Cadmium chloride","MIE","long description" 13
    """
    events = []
    ...
    for line in text.splitlines():
        ...
        chemical, event_type, long_desc, id_block = m.groups()
        # split trailing IDs → ["3", "4", "10"]
        pid_list = [int(x) for x in id_block.split()]

        # duplicate event entry for each paragraph
        for pid in pid_list:
            events.append({
                "chemical": chemical,
                "event_type": event_type,
                "event_short": "",
                "event_long": long_desc,
                "order": None,
                "paragraph_id": pid
            })

    # Assign proper order per paragraph
    by_pid = {}
    for ev in events:
        by_pid.setdefault(ev["paragraph_id"], []).append(ev)

    for pid, ev_list in by_pid.items():
...
class ParagraphBlock:
    ...
    def to_dict(self):
        ...
        paragraph_id = int(pid_raw) if pid_raw else None
        paragraph_text = self.paragraph_text.get("1.0", "end").strip()
        return {
            "paragraph_id": paragraph_id,
            "paragraph_text": paragraph_text,
            "events": []   # placeholder, to be filled after parsing
        }
...
class App:
    def __init__(self, root):
        ...
        tk.Label(extract_frame, text="Text to extract").pack(anchor="w")

        self.to_extract = tk.Text(extract_frame, width=120, height=10)
        self.to_extract.pack(fill="both", expand=True)
    ...
    def on_confirm(self):
        ...
        for block in self.paragraph_blocks:
            p = block.to_dict()
            p["events"] = []   # ensure exists
            paragraphs.append(p)

        # Parse extraction lines
        raw_text = self.to_extract.get("1.0", "end")
        events = parse_extraction_lines(raw_text)

        # Assign events by paragraph_id
        paragraphs_by_id = {p["paragraph_id"]: p for p in paragraphs}
        for ev in events:
            pid = ev["paragraph_id"]
            if pid in paragraphs_by_id:
                paragraphs_by_id[pid]["events"].append(ev)
