...
'''
Leftover issues:
- pdf divider redacts random parts of words (e.g. variabi ty, paper 1) 
- maybe find a way to remove lines that are a list of references
'''

INPUT_PATH = "data/raw/markdown/"
OUTPUT_PATH = "data/raw/markdown_clean/"
...
def find_broken_words_with_correction(
    text: str,
    max_distance: int = 2,
    min_joined_len: int = 6,
):
    """
    ...
    for i in range(len(tokens) - 1):
        ...
        unknown1 = w1.lower() not in spell
        unknown2 = w2.lower() not in spell
        ...
        candidate = spell.correction(joined)
        if not candidate:
            continue

        # Require small edit distance
        dist = levenshtein(joined, candidate)
...
def find_broken_words(text: str,
                      lang: str = "en",
                      min_joined_len: int = 6,
                      min_joined_freq: float = 1e-8):
    """
...
def fix_broken_words(text: str,
                     lang: str = "en",
                     min_joined_len: int = 6,
                     min_joined_freq: float = 1e-8) -> str:
    """
    Return text with broken words merged.
    """
    corrections = find_broken_words(
        text,
        lang=lang,
...
def normalize_coding(text):
    ...
    # Space before only -> space before (except ffi, ffl because can't be start of word)
    norm = norm.replace(' /uniFB01', ' fi')
    norm = norm.replace(' /uniFB02', ' fl')
    norm = norm.replace(' /uniFB03', 'ffi')
    norm = norm.replace(' /uniFB04', 'ffl')
...
def remove_meaningless_lines(text: str) -> str:
    """
...
def remove_bare_link_lines(text: str) -> str:
    """
...
def remove_trailing_dashes(text: str) -> str:
    """
...
def inspect_lines(text: str, indices):
    lines = text.splitlines()
    for i in indices:
        if 0 <= i < len(lines):
            line = lines[i]
...
def clean(md_text):
    ...
    text = remove_meaningless_lines(text)
    text = remove_bare_link_lines(text)
    ...
    text = join_broken_lines(text)
    text = remove_trailing_dashes(text)
